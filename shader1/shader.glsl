...
float mapTheWorld(vec3 p) {
  float distortionMultiplier = clamp(-0.0f, 1.7f * sin(fGlobalTime * timeScale) * sin(fGlobalTime), 1.7f) * 5.0f;
  ...
  return differenceCSG(sphere0 + distortion, cube0);
}
...
const vec3 ambientColor = vec3(0.9f, 0.9f, 0.0f);
...
void main(void)
{
  ...
  lightPosition = vec3(0.0f, 2.0f, -2.0f);
